...
<div class="container py-5">
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" id="title" class="@error('title') is-invalid @enderror form-control" value="{{ old('title', $product->title ?? '') }}" required>
            @error('title')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="price" id="price" step="0.01" min="0" class="@error('price') is-invalid @enderror form-control" value="{{ old('price', $product->price) }}" required>
            @error('price')
                <div class='invalid-feedback'>{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="description" id="description" rows="4" class="@error('description') is-invalid @enderror form-control">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <div class='invalid-feedback'>{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="pet" id="pet" class="@error('pet') is-invalid @enderror form-select" required>
                ...
                <option value="cat" {{ (old('pet', $product->pet) == 'cat') ? 'selected' : '' }}>Кошка</option>
                <option value="dog" {{ (old('pet', $product->pet) == 'dog') ? 'selected' : '' }}>Собака</option>
                <option value="bird" {{ (old('pet', $product->pet) == 'bird') ? 'selected' : '' }}>Птица</option>
            </select>
            @error('pet')
                <div class='invalid-feedback'>{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            ...
            <textarea name="shortDescription" id="shortDescription" rows="2" class="@error('shortDescription') is-invalid @enderror form-control">{{ old('shortDescription', $product->shortDescription) }}</textarea>
        ...
        <div class="mb-3">
            ...
            <textarea name="structure" id="structure" rows="3" class="@error('structure') is-invalid @enderror form-control">{{ old('structure', $product->structure) }}</textarea>
            @error('structure')
                <div class='invalid-feedback'>{{ $message }}</div>
            @enderror
        </div>
